...
function getStringLength(value) {
  ...
  // If the value is not a string or is undefined/null, return 0
  return 0;
}
...
function isPalindrome(str) {
  const str1 = str
    ...
    .replaceAll(',', '')
    .replaceAll('.', '');
  const strPol = str1.split('').reverse().join('');
  return str1 === strPol;
}
...
function encodeToRot13(str) {
  return str.replace(/[a-z]/gi, (res) =>
    String.fromCharCode(
      res.charCodeAt(0) + (res.toLowerCase() <= 'm' ? 13 : -13)
    )
  );
}
...
function getCardId(value) {
  ...
  if (value[0] === 'Q') res += 11;
  if (value[0] === 'K') res += 12;
  if (value.slice(-1) === '♦') res += 13;
  if (value.slice(-1) === '♥') res += 26;
  if (value.slice(-1) === '♠') res += 39;
  return res;
}
